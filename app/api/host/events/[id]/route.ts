...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const db = getDb();
    const [event] = await db.update(events).set({ ...(payload.status ? { status: payload.status } : {}), ...(typeof payload.orderingEnabled === "boolean" ? { orderingEnabled: payload.orderingEnabled } : {}), updatedAt: now() }).where(eq(events.id, id)).returning();
