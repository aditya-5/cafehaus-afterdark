...
import type { AdminSnapshot, DrinkRecord, InvitationRecord, OrderStatus, OrderView } from "../types";
import { customizationLine, formatClock, formatEventDate, oneWordTitleCase, requestJson } from "../lib/client";
...
export function AdminExperience() {
  ...
  if (!snapshot) return <AdminLogin pin={pin} onPin={setPin} loading={loading} error={loginError} onSubmit={() => void load(pin)} />;
...
function AdminLogin({ pin, onPin, onSubmit, loading, error }: { pin: string; onPin: (value: string) => void; onSubmit: () => void; loading: boolean; error: string }) {
  const submit = (event: FormEvent) => { event.preventDefault(); onSubmit(); };
  return <main className="event-app admin-login"><div className="grain" aria-hidden="true" /><section><Brand host /><p className="kicker">Private host access</p><h1>Your counter,<br /><em>locked.</em></h1><p className="intro">Enter the four-digit host PIN. Guest links never expose this console.</p><form onSubmit={submit}><label className="field-label">Host PIN<input value={pin} onChange={(event) => onPin(event.target.value.replace(/\D/g, "").slice(0, 4))} inputMode="numeric" type="password" /></label>{error && <p className="form-error">{error}</p>}<button className="primary-button" disabled={pin.length !== 4 || loading}>{loading ? "Opening…" : "Open host console"}</button></form></section></main>;
}
...
function TokenRoom({ snapshot, onAction }: { snapshot: AdminSnapshot; onAction: (action: "add_one" | "add_all" | "reset_all" | "fulfill_request", guestId?: string) => void }) {
  return <section className="host-tools token-room"><div className="panel-head"><div><p className="section-label">Coffee passes & plus ones</p><h2>Top up the room</h2><p>Tap any guest to add one token. Coral chips have requested more.</p></div><div className="host-tool-actions"><button className="secondary-button" onClick={() => onAction("add_all")}>+1 for everyone</button><button className="primary-button" onClick={() => onAction("reset_all")}>Reset all to 2</button></div></div><div className="balance-strip">{snapshot.guests.map((guest) => <button key={guest.id} className={`balance-chip ${guest.tokenRequestStatus === "requested" ? "requested" : ""}`} onClick={() => onAction(guest.tokenRequestStatus === "requested" ? "fulfill_request" : "add_one", guest.id)}><strong>{guest.firstName}</strong><span>{guest.tokenBalance} tokens</span>{guest.tokenRequestStatus === "requested" && <small>Requested</small>}</button>)}</div></section>;
}
...
function InvitationRoom({ snapshot, pin, onRefresh, onLatest, onToast }: { snapshot: AdminSnapshot; pin: string; onRefresh: () => Promise<void>; onLatest: (invite: { name: string; phone: string; url: string }) => void; onToast: (message: string) => void }) {
  ...
  const change = async (invitation: InvitationRecord, action: "rescind" | "reissue") => { try { const result = await adminRequest<{ inviteUrl?: string }>(pin, `/api/invitations/${invitation.id}`, { method: "PATCH", body: JSON.stringify({ action }) }); if (result.inviteUrl) onLatest({ name: invitation.invitedName, phone: invitation.invitedPhoneE164, url: result.inviteUrl }); await onRefresh(); onToast(action === "rescind" ? "Invitation rescinded" : "New link created; the old link is invalid"); } catch (error) { onToast(error instanceof Error ? error.message : "Could not update invitation"); } };
  return <section className="host-tools invite-manager"><p className="section-label">Invitation manager</p><h2>Invite someone</h2><p>Create the unique link, then send it through your own Messages app. Reissuing a link immediately invalidates the previous one.</p><form className="invite-create" onSubmit={create}><input value={name} onChange={(event) => setName(oneWordTitleCase(event.target.value))} placeholder="First name" required /><input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Mobile number" inputMode="tel" required /><button className="primary-button" disabled={sending}>{sending ? "Creating…" : "Create invite"}</button></form><div className="invitation-list">{snapshot.invitations.map((invitation) => <div className="invitation-row" key={invitation.id}><div><strong>{invitation.invitedName}</strong><small>{invitation.invitedPhoneE164}{invitation.parentGuestId ? " · +1" : ""}</small></div><span className={`invite-status ${invitation.status}`}>{invitation.status === "rsvped" ? "Accepted" : invitation.status}</span><div className="invitation-actions">{invitation.status !== "rescinded" && <button onClick={() => void change(invitation, "rescind")}>Rescind</button>}<button onClick={() => void change(invitation, "reissue")}>New link</button></div></div>)}</div></section>;
}
...
function InviteShare({ invite, onClose, onToast }: { invite: { name: string; phone: string; url: string }; onClose: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="modal-backdrop"><section className="drink-sheet share-sheet"><button className="close-button" onClick={onClose}>×</button><p className="section-label">Unique invitation ready</p><h2>Send it to {invite.name}.</h2><p className="sheet-description">This link is their bearer pass. Reissue it from the invitation manager if it is ever shared or lost.</p><div className="share-url">{invite.url}</div><div className="share-actions"><button className="secondary-button" onClick={() => void copy()}>Copy message</button><a className="primary-button link-button" href={`sms:${invite.phone}?&body=${encodeURIComponent(body)}`}>Open Messages</a></div></section></div>;
}
