...
function TokenRoom({ snapshot, onAction }: { snapshot: AdminSnapshot; onAction: (action: "add_one" | "add_all" | "reset_all" | "fulfill_request", guestId?: string) => void }) {
  return <section className="host-tools token-room"><div className="panel-head"><div><p className="section-label">Coffee passes & plus ones</p><h2>Top up the room</h2><p>Tap any guest to add one token. Coral chips have requested more.</p></div><div className="host-tool-actions"><button className="secondary-button" onClick={() => onAction("add_all")}>+1 for everyone</button><button className="primary-button" onClick={() => onAction("reset_all")}>Reset all to 2</button></div></div><div className="balance-strip">{snapshot.guests.map((guest) => { const plusOne = snapshot.invitations.find((invitation) => invitation.guestId === guest.id && invitation.parentGuestId); return <button key={guest.id} className={`balance-chip ${guest.tokenRequestStatus === "requested" ? "requested" : ""} ${plusOne ? "is-plus-one" : ""}`} onClick={() => onAction(guest.tokenRequestStatus === "requested" ? "fulfill_request" : "add_one", guest.id)}><span className="balance-name"><strong>{guest.firstName}</strong>{plusOne && <small>{plusOne.parentGuestName}’s +1</small>}</span><span>{guest.tokenBalance} tokens</span>{guest.tokenRequestStatus === "requested" && <small className="request-badge">Requested</small>}</button>; })}</div></section>;
}
...
function InviteShare({ invite, onClose, onToast }: { invite: { name: string; phone: string; url: string }; onClose: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="modal-backdrop"><section className="drink-sheet share-sheet"><button className="close-button" onClick={onClose}>×</button><p className="section-label">Unique invitation ready</p><h2>Send it to {invite.name}.</h2><p className="sheet-description">This link is their bearer pass. Reissue it from the invitation manager if it is ever shared or lost.</p><div className="share-url">{invite.url}</div><div className="share-actions share-grid"><button className="secondary-button" onClick={() => void copy()}>Copy message</button><a className="secondary-button link-button" href={`https://wa.me/?text=${encodeURIComponent(body)}`} target="_blank" rel="noreferrer">WhatsApp</a><a className="secondary-button link-button" href={`sms:${invite.phone}?&body=${encodeURIComponent(body)}`}>Messages</a><button className="primary-button" onClick={() => void share()}>Share…</button></div></section></div>;
}
